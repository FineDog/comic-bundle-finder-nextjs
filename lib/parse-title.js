 ...
 *
 * Negated terms (e.g. "-second", "-2nd") are pulled out as exclusions before any
 * other parsing, regardless of where they appear in the query. A leading hyphen is
 * only treated as a negation operator when preceded by whitespace or the start of
 * the string, so intra-word hyphens in series names ("spider-man") are preserved.
 *
 * @param {string} query
 * @returns {{ series: string, volume: number|null, issue: number|null, metadataFilters: string[], exclusions: string[] }}
 */
export function parseQuery(query) {
  let s = query.replace(/\([^)]*\)/g, " ");

  // Pull out negated terms first so they act as exclusions rather than required
  // series words or metadata filters. `(?:^|\s)-` ensures only a standalone leading
  // hyphen counts — the hyphen in "spider-man" (preceded by a letter) is untouched.
  const exclusions = [];
  s = s.replace(/(?:^|\s)-(\S+)/g, (_, term) => {
  ...
  return { series, volume, issue, metadataFilters, exclusions };
}
 ...
 *  - Exclusions: any negated term ("-second") that appears in the listing title
 *    rejects the listing. No exclusions → no exclusion filtering.
 *
...
export function titleMatchesQuery(listingTitle, queryString) {
  ...
  // Exclusions: any negated term present in the title rejects the listing.
