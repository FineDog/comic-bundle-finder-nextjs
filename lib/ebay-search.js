...
//
// zip: US postal code for accurate domestic/international-to-US shipping estimates.
// country: ISO country code (e.g. "GB") used when no zip is available — enough for
//          eBay to return zone-based international shipping estimates.
// visitorId: stable per-browser id so the Wave 1 search_logs row can be joined to
//            the matching Umami event for the same visitor. Optional (null = guide pages).
export async function runEbaySearch(issues, zip, {
  onWave1,
  onWave2Start,
  onWave2,
  onWave2End,
} = {}, country = null, visitorId = null) {
  const location = { zip: zip || null, country: country || null, visitorId: visitorId || null };

  // Wave 1
  const res = await fetch("/api/search", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ issues, ...location }),
  });
  ...
  try {
    const res2 = await fetch("/api/search", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ issueOffsets: wave2Tasks, ...location }),
    });
