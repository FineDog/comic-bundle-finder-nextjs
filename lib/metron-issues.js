...
export async function getMetronIssuesCached(metronId) {
  ...
  // NOTE: there is intentionally NO staleness/TTL gate here. Series issue lists are
  // effectively immutable (a completed run never gains issues), and the incremental
  // refresh job cycles the full ~16k-series catalog far slower than any short TTL.
  // A previous 30-day read TTL caused ~95% of series to return false "not indexed"
  // 503s once the initial bulk backfill aged past 30 days. cached_at is used ONLY by
  // the refresh job (oldest-first prioritization), never to hide valid data from
  // readers. Serving a slightly-stale-but-present issue list always beats a 503.
  //
  // Errors are NOT swallowed: a genuine DB failure must surface as a 500 so it is
  // distinguishable in logs from a genuinely-absent row (null → 503 notIndexed).
  const pool = await getPool();
  const { rows } = await pool.query(
    "SELECT issues FROM series_issues WHERE metron_id = $1",
    [Number(metronId)]
  );
  return rows.length ? rows[0].issues : null;
}
