...
// Matches ratio incentive variant notation in a listing title (e.g. "1:25", "1:100").
const RATIO_VARIANT_RE = /\b1\s*:\s*\d+\b/;
...
export async function searchForValuation(issueName) {
  ...
  // Do NOT filter by buyingOptions — "Best Offer" listings (BEST_OFFER type)
  // have a valid listed price and represent real market data, but are excluded
  // by a FIXED_PRICE filter even though they're not auctions.  Dropping this
  // filter recovers ~30% more listings, particularly cheap copies.
  // Keep conditions filter to exclude "For Parts / Not Working" etc.
