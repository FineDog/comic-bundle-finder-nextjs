...
export const STATUS = {
  ...
  BLOCKED_VARIANT:         "Blocked: variant edition",
...
// "Newsstand variant" and "direct variant" are standard edition descriptions,
// not special variants — exclude them from the variant block.
const STANDARD_VARIANT_RE = /\b(newsstand|direct)\s+variant\b/i;
...
function getBlockReason(title, issueName, queryMeta) {
  ...
  if (!queryIncludesRatio && RATIO_VARIANT_RE.test(title)) {
    return STATUS.BLOCKED_VARIANT;
  }

  // Block any listing with "variant" in the title unless:
  //  - The user's query itself includes "variant" (they're searching for one), OR
  //  - It's a "newsstand variant" or "direct variant" (standard edition descriptions)
  if (/\bvariant\b/i.test(title) && !STANDARD_VARIANT_RE.test(title)) {
    const queryIncludesVariant = queryMeta.some((f) => /\bvariant\b/i.test(f));
    if (!queryIncludesVariant) return STATUS.BLOCKED_VARIANT;
  }
