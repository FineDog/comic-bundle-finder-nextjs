...
function getBlockReason(title, issueName, queryYear) {
  ...
  if (parsed.issueSet.size > 1) return STATUS.BLOCKED_LOT;
...
export async function searchForValuation(issueName) {
  ...
  // Query exclusions (same approach as lib/ebay.js, extended for valuation).
  const exclusions =
    "-lot -set -run -collection -bundle -wholesale " +
    "-cgc -cbcs -pgx -facsimile -reprint -signed -omnibus -tpb";

  const params = new URLSearchParams({ q: `${issueName} ${exclusions}`, category_ids: CATEGORY_ID, limit: "200" });
