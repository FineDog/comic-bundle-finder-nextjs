...
//
// Uses the eBay Finding API (findCompletedItems). The Finding API has a tight
// per-second rate limit, so calls are always made sequentially with a delay
// between them — never concurrently.
//
...
// Minimum gap between Finding API calls to stay under the per-second rate limit.
...
export async function searchSoldListings(issueName, page = 1) {
  ...
  for (const item of rawItems) {
    ...
    items.push({ title, url: itemUrl, price: parseFloat(priceRaw), dateSold });
  }
...
// ---------------------------------------------------------------------------
// Always sequential — the Finding API rate-limits concurrent callers hard.
export async function searchSoldBatch(issues) {
  const results = [];
  for (let i = 0; i < issues.length; i++) {
    if (i > 0) await sleep(CALL_DELAY_MS);
    try {
      const result = await searchSoldListings(issues[i]);
      results.push({ issue: issues[i], ...result });
    } catch (err) {
      results.push({ issue: issues[i], items: [], total: 0, error: err.message });
    }
  }
  return results;
}
