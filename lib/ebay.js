...
// Runs parallel queries for all hyphenated/flat variants so neither token form
// is silently missed (eBay Browse treats "spider-man" and "spiderman" as distinct).
...
export function aggregateRows(issueListings) {
  ...
  for (const [seller, issuesFound] of Object.entries(sellerIssues)) {
    ...
    for (const [issueName, listings] of Object.entries(issuesFound)) {
      for (const listing of listings) {
        rows.push({ seller, bundle_count: bundleCount, issue: issueName, quantity: listing.quantity || 1, ...listing });
      }
    }
  }
