...
export async function searchEbay(token, issueName, maxPrice) {
...
export async function searchEbayBatch(token, issues, maxPrice, concurrency = 8) {
  const allResults = [];
  for (let i = 0; i < issues.length; i += concurrency) {
    const batch = issues.slice(i, i + concurrency);
    const batchResults = await Promise.all(
      batch.map((issue) => searchEbay(token, issue.issueName, maxPrice))
    );
