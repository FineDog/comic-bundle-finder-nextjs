...
// Returns the canonical (hyphenated) version of issueName if any word in
// it matches a dehyphenated form from HYPHENATED_WORDS, otherwise null.
function getCanonicalQueryName(issueName) {
  ...
  for (const word of HYPHENATED_WORDS) {
    ...
    if (lower.includes(word)) continue; // already has the hyphen
...
// Runs a second eBay query with the canonical hyphenated form when the user
// omitted a hyphen (e.g. "spiderman" -> also queries "spider-man").
